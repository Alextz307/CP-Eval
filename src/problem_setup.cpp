...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    std::vector<std::string> templates = {
        "main.cpp",         "brute.cpp",
        "generator.cpp",    "input_validator.cpp",
        "check_inputs.cpp", "genlib.hpp",
        "stress_test.cpp",  "eval.cpp",
        "validator.cpp",    "compile.sh"};

    for (const auto& t : templates) {
      copyTemplate(t, problemPath);
    }

    fs::permissions(problemPath / "compile.sh", fs::perms::owner_exec |
                                                    fs::perms::owner_read |
                                                    fs::perms::owner_write);

    std::cout << "Created problem environment: " << problemName << std::endl;
    std::cout << "Run 'cd " << problemName
              << " && ./compile.sh' to get started." << std::endl;
