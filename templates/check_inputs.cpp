...
int main(int argc, char* argv[]) {
    ...
    if (validator.substr(0, 2) != "./" && validator[0] != '/') validator = "./" + validator;

    if (!fs::exists(validator)) {
        cerr << "Validator executable not found: " << validator << endl;
        cerr << "Compile it first: g++ input_validator.cpp -o input_validator" << endl;
        return 1;
    }
    ...
    cout << "Checking " << inputs.size() << " inputs with " << validator << "..." << endl;

    for (const auto& input_path : inputs) {
        string command = validator + " < \"" + input_path.string() + "\" > /dev/null 2>&1";
        ...
        if (ret == 0) {
            ...
            cout << "[FAIL] " << input_path.filename() << " (Exit Code: " << ret << ")" << endl;
            failed++;
        }
    }
